...
const [locationData, setLocationData] = useState([]);
 ...
 useEffect(() => {
  axios
    .get("https://space-analysis-backend.onrender.com/api/launch-summary-by-country")
    .then((res) => {
...
const toggleCountry = async (country) => {
  ...
  try {
    const res = await axios.get(`https://space-analysis-backend.onrender.com/api/launch-sites-by-country?country=${country}`);
...
{expandedCountry === row.country && Array.isArray(locationData) && locationData.map((loc, i) => (
      <tr key={`${row.country}-loc-${i}`} className="bg-gray-50 dark:bg-slate-700 text-sm">
